...
import torch
import numpy as np
import src.BinAttack_model as attack_model
...
model_dir = root_dir + '/dataset/'+args.dataset+'/BinarizedAttack/' + 'rand'+ str(args.size) + '_' + str(args.trial) + '/saved_ckt'
save_dir = root_dir + '/dataset/'+args.dataset+'/BinarizedAttack/' + 'rand'+ str(args.size) + '_' + str(args.trial) + '/sort_ckt'
...
for lam in lam_lst:
    model = attack_model.multiple_AS(target_node_lst, n, args.device).to(args.device)
    ...
    for i in range(args.epoch):
        ...
        for p in model.parameters():
            p.grad *= model.filter_potential_singletons(triple_torch)
        optimizer.step()
